...
export default class CommonHelper {
	 ...
	 * @return {Boolean}
	 */
	static hasNonEmptyProps<T>(obj: GenericObject<T>): boolean {
		for (const i in obj) {
	 ...
	 * @return {Boolean}
	 */
	static isObjectArrayWithKeys<T>(arr: Array<GenericObject<T>>, keys: Array<any>): boolean {
		...
		const normalizedKeys = CommonHelper.toArray(keys);
	 ...
	 * @return {Array}
	 */
	static toArray<T>(arr: Array<T>, allowEmpty = false): Array<T> {
	 ...
	 * @return {Boolean}
	 */
	static inArray<T>(arr: T[], value: T): boolean {
	 ...
	 * @param {Mixed} value
	 */
	static removeByValue<Mixed>(arr: Array<Mixed>, value: Mixed) {
		for (let i = arr.length - 1; i >= 0; i--) {
			if (arr[i] == value) {
				arr.splice(i, 1);
				break;
	 ...
	 * @param {Mixed} value
	 */
	static pushUnique<Mixed>(arr: Array<Mixed>, value: Mixed) {
	 ...
	 * @return {Object}
	 */
	static findByKey<T, K extends keyof T>(objectsArr: Array<T>, key: K, value: T[K] | T): T | null {
		for (const i in objectsArr) {
	 ...
	 * @return {Object}
	 */
	static groupByKey<T, K extends keyof T>(objectsArr: Array<T>, key: K): any {
		const result: any = {};
	 ...
	 * @param {T}  value
	 */
	static removeByKey<T>(objectsArr: Array<{ [K: string]: T }>, key: string, value: T) {
		for (const i in objectsArr) {
			if (objectsArr[i][key] == value) {
	 ...
	 * @return {Array}
	 */
	static pushOrReplaceByKey<T>(
		objectsArr: Array<{ [K: string]: T }>,
		item: { [K: string]: T },
		key = 'id'
	): Array<{ [K: string]: T }> | undefined {
	 ...
	 * @return {Array}
	 */
	static filterDuplicatesByKey(
		objectsArr: Array<GenericObject<string>>,
		key = 'id'
	): Array<GenericObject<string>> {
		const uniqueMap: GenericObject<GenericObject<string>> = {};
	 ...
	 * @return {Object}
	 */
	static filterRedactedProps(obj: object, mask = '******'): object {
	 ...
	 * @return {Mixed}
	 */
	static getNestedVal<T>(
		data: T | Array<T>,
		path: string,
		defaultVal = null,
		delimiter = '.'
	): any {
		let result: T | Array<T> | any = data || {};
	 ...
	 * @param  {String}       delimiter
	 */
	static setByPath(data: Array<any> | any, path: string, newValue: any, delimiter = '.') {
		...
		if (!path) {
			console.warn('setByPath: path is empty.');
			return;
		}
	 ...
	 * @param  {String}       delimiter
	 */
	static deleteByPath(data: Array<any> | object, path: string, delimiter = '.') {
		let result = data || {};
		const parts = path.split(delimiter);
		const lastPart = parts.pop();
	 ...
	 * @return {String}
	 */
	static slugify(str: string, delimiter = '_', preserved: Array<any> = ['.', '=', '-']): string {
		if (str === '') {
			return '';
		}
	 ...
	 * @return {Array}
	 */
	static splitNonEmpty(str: string, separator = ','): Array<any> {
	 ...
	 * @return {Array}
	 */
	static joinNonEmpty(items: string, separator = ', '): Array<any> {
		const result = [];
	 ...
	 * @return {DateTime}
	 */
	static getDateTime(date: string | Date): DateTime {
	 ...
	 * @return {String}
	 */
	static formatToUTCDate(date: string | Date, format = 'yyyy-MM-dd HH:mm:ss'): string {
	 ...
	 * @return {String}
	 */
	static formatToLocalDate(date: string | Date, format = 'yyyy-MM-dd HH:mm:ss'): string {
	 ...
	 * @return {Promise}
	 */
	static async copyToClipboard(text: string): Promise<any> {
	 ...
	 */
	static openInWindow(url: string, width: number, height: number, name: string): object {
		width = width || 1024;
		height = height || 768;
		name = name || 'popup';
	...
	static getQueryString(url: string): string {
		...
		return url.substring(
			queryStartPos + 1,
			hashStartPos > queryStartPos ? hashStartPos : url.length
		);
	}
	 ...
	 * @return {Object}
	 */
	static getQueryParams(url: string): object {
		const result = {};
		const params = CommonHelper.getQueryString(url).split('&');
	 ...
	 * @return {String}
	 */
	static setQueryParams(url: string, params: object, extend = true): string {
		const oldQueryString = CommonHelper.getQueryString(url);
		const oldParams = extend && oldQueryString ? CommonHelper.getQueryParams(url) : {};
		const resultParams = Object.assign(oldParams, params);
		let newQueryString = '';

		for (const param in resultParams) {
	 ...
	 * @param {Object} params
	 */
	static replaceClientQueryParams(params: object) {
		const url = CommonHelper.setQueryParams(window.location.href, params);
	 ...
	 * @return {Object}
	 */
	static getJWTPayload(jwt: string): object {
	 ...
	 * @return {Boolean}
	 */
	static hasImageExtension(filename: string): boolean {
	 ...
	 * @return {Promise}
	 */
	static checkImageUrl(url: string): Promise<any> {
	 ...
	 * @return {Promise}
	 */
	static generateThumb(file: File, width = 100, height = 100): Promise<any> {
	 ...
	 * @param {String} [suffix]
	 */
	static setDocumentTitle(title: string, suffix = 'PocketBase') {
		const parts = [];
	 ...
	 * @param {mixed}    value
	 */
	static addValueToFormData(formData: FormData, key: string, value: any) {
	 ...
	 * @return {Object}
	 */
	static defaultFlatpickrOptions(): object {
		return {
	 ...
	 * @return {Object}
	 */
	static dummyCollectionRecord(collection: object): object {
	 ...
	 * @return {String}
	 */
	static getFieldTypeIcon(type: string): string {
	 ...
	 * @return {String}
	 */
	static getFieldValueType(field: object): string {
		field = field || {};
